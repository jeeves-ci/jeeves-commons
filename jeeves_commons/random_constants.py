...
# This implementation was blatantly copied from Moby and translated to python:
...
def get_random_name(retry):
    name = '{left}_{right}'.format(left=left[random.randint(0, len(left)-1)],
                                   right=right[random.randint(0, len(right)-1)])
    if name == "boring_wozniak": # Steve Wozniak is not boring
        get_random_name(retry + 1)

    if retry > 0:
        name = '{name}{num}'.format(name=name, num=random.randint(0, 10))
